...
| Source depth and context | **Implemented**: explicit `includeContent` uses the local safe fetcher with a finite 1–20 page count, up to 32,000 requested characters per page, and a 45-KB search output budget | Optional bounded fetch of selected sources with clean excerpts, preserved URLs, and explicit truncation/provenance |
